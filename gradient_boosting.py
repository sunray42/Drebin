...
def gradientboost(x_train, y_train, x_test, y_test, TUNING = False):
    if TUNING:
        ...
        for score in scores:
            ...
            clf = GridSearchCV(GradientBoostingClassifier(), tuned_parameters, cv=5, scoring=score, n_jobs=-1)
        ...
        models = [
            GradientBoostingClassifier(max_depth=5, n_estimators=1000, subsample=0.7)
        ]
        for model in models:
...
if __name__ == "__main__":
    ...
    y_pred_train, y_pred_test, pred_proba_test = gradientboost(x_train, y_train, x_test, y_test, TUNING=True)
    ...
    fpr_gb, tpr_gb, threshold_gb = roc_curve(y_test, pred_proba_test)
    roc_auc = auc(fpr_gb, tpr_gb)
    plt.plot(fpr_gb, tpr_gb, color='darkorange', lw=2,
             label='gradient_boosting ROC curve (area = %0.2f)' % roc_auc)
